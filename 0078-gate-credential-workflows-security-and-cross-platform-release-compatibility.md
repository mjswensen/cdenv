...
- Added a bounded controlled smart-HTTP TLS server with a private test CA and hostname-valid certificate. The packaged workflow performs real first-hook HTTPS clone, fetch, push, uncached token rotation, denied-origin checks, and secret-token scans without disabling certificate verification or contacting a public Git service.
